...
import { deleteClassType, fetchAllClassTypes } from '../../redux/classtypes/classtypeSlice';
import { getComponentPermissions } from '../../config/utils/permission';

function ClassTypes() {
  const history = useHistory();
  const dispatch = useDispatch();
  const { classtypes, loading } = useSelector((state) => state.classtypes);
  const { login: user } = useSelector((state) => state.auth);
  const { canAdd, canEdit, canDelete } = getComponentPermissions(user, 'ClassTypes');
  ...
  const { notData, visible, selectedClassType } = state;

  const handleEdit = (classtype) => {
    const { _id: id, ...rest } = classtype;
  ...
  const handleDelete = (id) => {
    dispatch(deleteClassType(id, selectedBranchId));
  ...
  useEffect(() => {
    if (classtypes && Array.isArray(classtypes)) {
      ...
      const formatted = paginatedData.map((classtype) => {
        const { _id, id, code, name, status, action } = classtype;
        return {
          key: _id || id,
          id: _id || id,
          code,
  ...
  }, [classtypes, pagination, searchTerm, sortStatus]);
    ...
    <>
      <ProjectHeader>
        <PageHeader
          ghost
          title="Class Types"
          subTitle={<>{loading ? 'Loading...' : `${dataSource.length} ClassTypes`}</>}
          buttons={[
            <Button disabled={!canAdd} onClick={showModal} key="1" type="primary" size="default">
              <FeatherIcon icon="plus" size={16} /> Create ClassType
            </Button>,
          ]}
        />
      </ProjectHeader>
            ...
            <ProjectSorting>
              <div className="project-sort-bar">
                <div className="project-sort-search">
                  <AutoComplete onSearch={handleSearch} dataSource={notData} placeholder="Search classtypes" patterns />
                </div>
