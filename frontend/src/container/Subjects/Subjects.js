...
import { deleteSubjects, fetchAllSubjects } from '../../redux/subjects/subjectsSlice';
...
function Subjects() {
  ...
  const { canAdd, canEdit, canDelete } = getComponentPermissions(user, 'subjects');
  ...
  const [state, setState] = useState({
    ...
    selectedSubjectsId: null,
  });
  const [searchTerm, setSearchTerm] = useState('');
  const [sortStatus, setSortStatus] = useState('category');

  const { notData, visible, selectedSubjects } = state;

  const handleEdit = (subjects) => {
    const { _id: id, ...rest } = subjects;
  ...
  const handleDelete = (id) => {
    dispatch(deleteSubjects(id, selectedBranchId));
  ...
  useEffect(() => {
    if (subjects && Array.isArray(subjects)) {
      ...
      const formatted = paginatedData.map((subjects) => {
        const { _id, id, code, name, status } = subjects;
        return {
          key: _id || id,
          id: _id || id,
          name,
  ...
  }, [subjects, pagination, searchTerm, sortStatus]);
    ...
    <>
      <ProjectHeader>
        <PageHeader
          ghost
          title="Subjects"
          subTitle={<>{loading ? 'Loading...' : `${dataSource.length} Subjects`}</>}
          buttons={[
            <Button disabled={!canAdd} onClick={showModal} key="1" type="primary" size="default">
              <FeatherIcon icon="plus" size={16} /> Create Subjects
            </Button>,
          ]}
        />
      </ProjectHeader>
            ...
            <ProjectSorting>
              <div className="project-sort-bar">
                <div className="project-sort-search">
                  <AutoComplete onSearch={handleSearch} dataSource={notData} placeholder="Search subjects" patterns />
                </div>
