...
function Customers() {
  ...
  useEffect(() => {
    if (customers && Array.isArray(customers)) {
      ...
      const formatted = paginatedData.map((customer) => {
        const { _id, id, name, email, address, phone } = customer;
        return {
          key: _id || id,
          id: _id || id,
          name,
          ...
          action: (
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
              <button type="button" onClick={() => handleEdit(customer)} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 30, height: 30, borderRadius: 6, border: '1px solid #E5E7EB', background: '#fff', cursor: 'pointer', color: '#2D3142' }} title="Edit"><EditOutlined style={{ fontSize: 14 }} /></button>
  ...
  }, [customers, pagination, searchTerm, sortStatus]);
    ...
    <ScreenWrap>
      <ProjectHeader>
        <PageHeader
          ghost
          title="Customers"
          subTitle={<>{loading ? 'Loading...' : `${dataSource.length} Customers`}</>}
          buttons={[
            <Button disabled={!canAdd} onClick={showModal} key="1" type="primary" size="default">
              <FeatherIcon icon="plus" size={16} /> Create Customer
            </Button>,
          ]}
        />
      </ProjectHeader>
