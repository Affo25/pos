...
import { deleteStudent, fetchAllStudents } from '../../redux/students/studentSlice';
...
function Students() {
  ...
  const { canAdd, canEdit, canDelete } = getComponentPermissions(user, 'Students');
  ...
  const { notData, visible, selectedStudent } = state;

  const showModal = () => {
    history.push(`${path}/studentSetup`);
  };

  const handleEdit = (student) => {
    history.push(`${path}/studentSetup/${student.id || student._id}`);
  };

  const handleDelete = (id) => {
    dispatch(deleteStudent(id, selectedBranchId));
  ...
  useEffect(() => {
    if (students && Array.isArray(students)) {
      ...
      const formatted = paginatedData.map((student) => {
        const { _id, id, name, fatherName, rollNo } = student;
        return {
          key: _id || id,
          id: _id || id,
          name,
  ...
  }, [students, pagination, searchTerm, sortStatus]);
            ...
            <ProjectSorting>
              <div className="project-sort-bar">
                <div className="project-sort-search">
                  <AutoComplete onSearch={handleSearch} dataSource={notData} placeholder="Search students" patterns />
                </div>
