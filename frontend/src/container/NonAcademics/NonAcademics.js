...
import { deleteNonAcademic, fetchAllNonAcademics } from '../../redux/nonacademics/nonacademicSlice';
import { getComponentPermissions } from '../../config/utils/permission';

function NonAcademics() {
  const history = useHistory();
  const dispatch = useDispatch();
  const { nonacademics, loading } = useSelector((state) => state.nonacademics);
  const { login: user } = useSelector((state) => state.auth);
  const { canAdd, canEdit, canDelete } = getComponentPermissions(user, 'NonAcademics');
  ...
  const [state, setState] = useState({
    ...
    visible: false,
    categoryActive: 'all',
    selectedNonAcademic: null,
    selectedNonAcademicId: null,
  });
  const [searchTerm, setSearchTerm] = useState('');
  const [sortStatus, setSortStatus] = useState('category');

  const { notData, visible, selectedNonAcademic } = state;

  const handleEdit = (nonacademic) => {
    const { _id: id, ...rest } = nonacademic;
  ...
  const handleDelete = (id) => {
    dispatch(deleteNonAcademic(id, selectedBranchId));
  ...
  useEffect(() => {
    if (selectedBranchId) {
      dispatch(fetchAllNonAcademics(selectedBranchId));
    }
  }, [dispatch, selectedBranchId]);

  useEffect(() => {
    if (nonacademics && Array.isArray(nonacademics)) {
      let filtered = [...nonacademics];
      ...
      const formatted = paginatedData.map((nonacademic) => {
        const { _id, id, name, fatherName, contactNo,  status } = nonacademic;
        return {
          key: _id || id,
          id: _id || id,
          name,
              ...
              overlay={
                <Menu className="custom-dropdown-menu">
                  <Menu.Item
                    ...
                    onClick={() => handleEdit(nonacademic)}
                  >
  ...
  }, [nonacademics, pagination, searchTerm, sortStatus]);
    ...
    <>
      <ProjectHeader>
        <PageHeader
          ...
          subTitle={<>{loading ? 'Loading...' : `${dataSource.length} Non Academic Staff`}</>}
          buttons={[
            <Button disabled={!canAdd} onClick={showModal} key="1" type="primary" size="default">
              <FeatherIcon icon="plus" size={16} /> Create Non Academic Staff
            </Button>,
          ]}
        />
      </ProjectHeader>
      <Main>
              ...
              <div className="project-sort-bar">
                <div className="project-sort-search">
                  <AutoComplete
                    ...
                    dataSource={notData}
                    placeholder="Search nonacademics"
                    patterns
                  />
                </div>
        ...
        <CreateNonAcademic
          visible={visible}
          onCancel={onCancel}
          nonacademic={selectedNonAcademic}
          onSuccess={() => {
            dispatch(fetchAllNonAcademics(selectedBranchId));
          }}
        />
