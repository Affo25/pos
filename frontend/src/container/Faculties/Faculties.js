...
import CreateFaculties from './CreateFaculties';
...
import { deleteFaculties, fetchAllFaculties } from '../../redux/faculties/facultiesSlice';
...
function Faculties() {
  ...
  const [state, setState] = useState({
    ...
    selectedFacultiesId: null,
  });
  const [searchTerm, setSearchTerm] = useState('');
  const [sortStatus, setSortStatus] = useState('category');

  const { notData, visible, selectedFaculties } = state;

  const handleEdit = (faculties) => {
    const { _id: id, ...rest } = faculties;
  ...
  useEffect(() => {
    if (faculties && Array.isArray(faculties)) {
      ...
      const formatted = paginatedData.map((faculties) => {
        const { _id, id, name, code, status } = faculties;
        return {
          key: _id || id,
          id: _id || id,
          name,
  ...
  }, [faculties, pagination, searchTerm, sortStatus]);
    ...
    <>
      <ProjectHeader>
        <PageHeader
          ghost
          title="Faculties"
          subTitle={<>{loading ? 'Loading...' : `${dataSource.length} Faculties`}</>}
          buttons={[
            <Button disabled={!canAdd} onClick={showModal} key="1" type="primary" size="default">
              <FeatherIcon icon="plus" size={16} /> Create Faculties
            </Button>,
          ]}
        />
      </ProjectHeader>
            ...
            <ProjectSorting>
              <div className="project-sort-bar">
                <div className="project-sort-search">
                  <AutoComplete onSearch={handleSearch} dataSource={notData} placeholder="Search faculties" patterns />
                </div>
