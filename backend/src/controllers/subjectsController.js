...
exports.createSubjects = async (req, res) => {
  try {
    ...
    const newSubjects = new Subjects(data);
...
exports.getSubjectss = async (req, res) => {
  try {
    ...
    const subjectss = await Subjects.find(query);
    res.status(200).json(subjectss);
  } catch (err) {
...
const checkFacultyAccess = async (id, user) => {
  const faculty = await Subjects.findById(id);
...
exports.updateSubjects = async (req, res) => {
  try {
    ...
    const updated = await Subjects.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
